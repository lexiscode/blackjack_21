...
def calculate_score(cards):
  ...
  # we can say, if 11 in cards and 10 in cards and len(cards) == 2:
  # but below is a better alternate way of writing the same thing  
...
#the whole game is stored in this function below, in order to loop it
def play_game(): 
  ...
  for i in range(2):
    user_cards.append(deal_card()) # adds random card (1st function call)
    computer_cards.append(deal_card()) # adds another random card (2nd function call)

   #user_score = calculate_score(user_cards)
   #computer_score = calculate_score(computer_cards)
   # but we'd store these two variables within a while loop, as show below
  ...
  while not is_game_over:
    ...
    if user_score == 0 or computer_score == 0 or user_score > 21:
      is_game_over = True # stops the loop, ends game
    else:
      # if the game has not ended, ask the user if they want to draw another card. If yes, then use the deal_card() function to add another 
  ...
  while computer_score != 0 and computer_score < 17:
    ...
    computer_score = calculate_score(computer_cards)
  # lets reveal the computer's final hands especially, alongside the users' as written below  
...
while input("Do you want to play a game of Blackjack? Type 'y' or 'n': ") == "y":
  clear()
  play_game() # to reply the whole game from start
